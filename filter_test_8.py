"""
FILTER TEST #8 — LUNAR CYCLE PHASE FILTER
==========================================
...
Academic Basis:
  Dichev & Janes (2001) "Lunar Cycle Effects in Stock Returns" — Journal of Business
  Van der Sar (2013) — Gold shows STRONGER lunar effects than equities
  Gold cultural link: physical demand tied to lunar calendars (Chinese New Year, 
  Diwali, Ramadan) → institutional buying pressure follows lunar cycles

Filter #8 Rule:
  AVOID trades when lunar illumination > 85% (full moon ± ~3 days)
  → Research shows returns are suppressed & momentum reversals more likely near full moon
  PREFER waxing phase (0-50% illumination) for LONG trades
  PREFER waning phase (50-99% illumination) for SHORT trades

What we test:
  Test A: Full moon avoidance only (illumination > 85% → skip)
  Test B: Full moon avoidance + waxing/waning directional bias
  → We'll run Test A first (most conservative, most academically supported)
"""
...
FULL_MOON_THRESHOLD = 85.0   # illumination % — above this = near full moon = avoid

# ── Compute Lunar Phase for every date ───────────────────────────────────────
print("Computing lunar phases via ephem library...")

def get_lunar_illumination(date_str):
    """Returns lunar illumination 0-100 (0=new moon, 100=full moon)"""
    m = ephem.Moon()
    m.compute(date_str)
    return m.phase   # 0-100

# Build a daily lunar phase table for our date range
dates = pd.date_range('2021-03-01', '2026-03-16', freq='D')
lunar_df = pd.DataFrame({'date': dates})
lunar_df['lunar_pct'] = lunar_df['date'].apply(
    lambda d: get_lunar_illumination(d.strftime('%Y/%m/%d')))

# Classify phases
lunar_df['lunar_phase_name'] = pd.cut(
    lunar_df['lunar_pct'],
    bins=[0, 10, 50, 85, 100],
    labels=['New Moon\n(0-10%)', 'Waxing\n(10-50%)', 'Waning\n(50-85%)', 'Full Moon\n(85-100%)'])

# Full moon avoidance flag (shifted by 1 day to prevent lookahead)
lunar_df['full_moon_zone'] = lunar_df['lunar_pct'] > FULL_MOON_THRESHOLD
# Waxing = illumination increasing (new → full)
lunar_df['waxing'] = lunar_df['lunar_pct'] < 50
lunar_df['waning'] = (lunar_df['lunar_pct'] >= 50) & (lunar_df['lunar_pct'] <= FULL_MOON_THRESHOLD)

# Shift by 1 day — use yesterday's moon phase (no lookahead)
lunar_df['full_moon_avoid'] = lunar_df['full_moon_zone'].shift(1).fillna(False)
lunar_df['waxing_prev']     = lunar_df['waxing'].shift(1).fillna(False)
lunar_df['waning_prev']     = lunar_df['waning'].shift(1).fillna(False)
lunar_df['lunar_pct_prev']  = lunar_df['lunar_pct'].shift(1).fillna(50)
lunar_df.set_index('date', inplace=True)

print(f"  Full moon zone days (>85%): {lunar_df['full_moon_zone'].sum()} days "
      f"({lunar_df['full_moon_zone'].mean()*100:.1f}%)")
print(f"  Waxing days (0-50%): {lunar_df['waxing'].sum()} days")
print(f"  Waning days (50-85%): {lunar_df['waning'].sum()} days")

# Show next 5 full moons approx (for reference)
full_moon_dates = lunar_df[lunar_df['full_moon_zone']].index[:10]
print(f"  Full moon windows detected: {list(full_moon_dates[:5].strftime('%Y-%m-%d'))}...")
...
# Merge lunar data
df = df.merge(lunar_df[['full_moon_avoid','waxing_prev','waning_prev','lunar_pct_prev']],
              left_on='date', right_index=True, how='left')
df['full_moon_avoid'] = df['full_moon_avoid'].fillna(False)
df['waxing_prev']     = df['waxing_prev'].fillna(True)   # default waxing if unknown
df['waning_prev']     = df['waning_prev'].fillna(False)

print(f"  Bars in full moon avoidance zone: {df['full_moon_avoid'].sum():,} "
      f"({df['full_moon_avoid'].mean()*100:.1f}%)")
...
# ✅ Filter #8: Avoid full moon zone
df['lunar_ok'] = ~df['full_moon_avoid']   # True = NOT in full moon avoidance zone

df['entry_long'] =(df['in_long_tap']  &(df['score_long']>=5)  &df['valid_session']&
                   df['macro_long']   &df['lunar_ok'])
df['entry_short']=(df['in_short_tap'] &(df['score_short']>=5) &df['valid_session']&
                   df['macro_short']  &df['lunar_ok'])

# ── Simulation (F#1 + F#3 + F#8) ────────────────────────────────────────────
print(f"Running backtest F#1 + F#3 + F#8 (full moon avoidance >{FULL_MOON_THRESHOLD}%)...")
...
tdf.to_csv('xauusd_filter8_trades.csv',index=False)
...
# Win rates by lunar phase (analysis)
tdf_m=tdf.copy()
tdf_m['date']=tdf_m['exit_time'].dt.normalize()
tdf_m=tdf_m.merge(lunar_df[['lunar_pct_prev']],left_on='date',right_index=True,how='left')
tdf_m['phase_bin']=pd.cut(tdf_m['lunar_pct_prev'],
    bins=[0,10,25,50,75,90,100],
    labels=['New\n(0-10)','Crescent\n(10-25)','Waxing\n(25-50)',
            'Gibbous\n(50-75)','Waning\n(75-90)','Full\n(90-100)'])
wr_by_phase = tdf_m.groupby('phase_bin')['status'].apply(
    lambda x: (x=='win').sum()/len(x)*100).round(1)
net_by_phase = tdf_m.groupby('phase_bin')['pnl'].sum()/INITIAL*100
...
print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #8: LUNAR CYCLE — FULL MOON AVOIDANCE (>{FULL_MOON_THRESHOLD}% illumination)
  Using ephem library | Shift-1 applied (no lookahead)
╠══════════════════════╦══════════════╦═══════════════════════════╣
  Metric              │ F#1+F#3 Base │ F#1+F#3+F#8 Result
╠══════════════════════╬══════════════╬═══════════════════════════╣
...
╠══════════════════════╩══════════════╩═══════════════════════════╣
  Win Rate by Lunar Phase (all trades analysed):""")

for phase in wr_by_phase.index:
    wr_p = wr_by_phase.get(phase, 0); net_p = net_by_phase.get(phase, 0)
    icon = '🌑' if 'New' in str(phase) else '🌕' if 'Full' in str(phase) else '🌙'
    print(f"    {icon} {str(phase):<20}: WR={wr_p:.1f}%  Net={net_p:+.2f}%")
...
fig.suptitle(
    f"Filter #8: Lunar Cycle — Full Moon Avoidance (>{FULL_MOON_THRESHOLD}% illumination)  [F#1+F#3+F#8]\n"
    f"Net: {net/INITIAL*100:+.2f}%  ·  MaxDD: {max_dd:.1f}%  ·  Sharpe: {sharpe:.2f}  ·  "
    f"WR: {wr:.1f}%  ·  PF: {pf:.2f}×  ·  Trades: {len(tdf):,}  ({1742-len(tdf)} blocked)\n"
...
ax0.set_title(f'Equity Curve — F#1+F#3+F#8  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
...
# Lunar Phase Win Rate
ax5=fig.add_subplot(gs[3,1])
phases=list(wr_by_phase.index); wr_vals=list(wr_by_phase.values)
moon_colors=['#1a1a3e','#2e4a7a','#4a7fb5','#b5d5f5','#7a9ec0','#ffd700']
bars=ax5.bar(phases,wr_vals,color=moon_colors[:len(phases)],edgecolor='#444',lw=0.5,zorder=3)
ax5.axhline(30.08,color='#ff9800',lw=1.5,ls='--',alpha=0.8,label='Baseline WR 30.08%')
ax5.set_title('Win Rate by Lunar Phase 🌙\n(all bars in dataset)',
              fontsize=9,fontweight='bold',color='white',pad=6)
ax5.set_ylabel('Win Rate %',color='#aaa',fontsize=7); ax5.tick_params(colors='#bbb',labelsize=7)
ax5.set_facecolor('#0d0d1f'); ax5.spines[:].set_color('#333'); ax5.grid(axis='y',alpha=0.15,zorder=1)
ax5.legend(fontsize=7,facecolor='#1a1a2e',labelcolor='white')
for bar,val in zip(bars,wr_vals):
    ax5.text(bar.get_x()+bar.get_width()/2,val+0.3,f"{val:.1f}%",
...
sns.heatmap(wk_m,cmap='RdYlGn',center=0,vmin=-vmax,vmax=vmax,
            ax=ax6,linewidths=0.2,cbar_kws={'label':'Net %','shrink':0.8})
...
out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter8_lunar.png'

"""
FILTER TEST #9 — GANN SQUARE OF 9 PRICE LEVELS
================================================
Builds on: Filter #1 + Filter #3 + Filter #8 (Lunar)
Active baseline: Trades=1,299 · WR=30.79% · PF=1.14x · Net=+109.68% · MaxDD=22.75% · Sharpe=0.80

WD Gann Square of 9 Mathematics:
  Gann discovered prices move in a spiral pattern.
  In sqrt-space, each 360° rotation = +1.0 unit
  Each 90° cardinal level = +0.25 unit

  sqrt(price) → add/subtract multiples of 0.25 → square the result
  
  Example for gold at $2000:
    sqrt(2000) = 44.72
    Gann levels: (44.72 ± n×0.25)² for n=1,2,3...
    = $2012.5, $2025.1, $2037.7... (every +90° in Gann geometry)

Filter Rule:
  Check if EMA21 is within 0.25% of a Gann Square of 9 level
  → EMA21 near a Gann level = geometric confluence with technical level
  → Much higher probability the level acts as true support/resistance

  Implemented as:
    sq = sqrt(ema21)
    nearest_gann = round(sq / 0.25) * 0.25   (snap to nearest cardinal)
    gann_level = nearest_gann ** 2
    dist_pct = abs(ema21 - gann_level) / ema21 × 100
    ENTER if dist_pct < 0.25% (EMA21 is at a Gann level)

Also compute: is CLOSE PRICE within 0.5% of a Gann level (price at geometric pivot)?
"""
...
GANN_UNIT = 0.25   # each 90° cardinal = 0.25 in sqrt-space

def gann_nearest_level(price):
    """Return the nearest Gann Square of 9 level to a given price"""
    sq     = np.sqrt(np.maximum(price, 0.01))
    snap   = np.round(sq / GANN_UNIT) * GANN_UNIT
    return snap ** 2

def gann_distance_pct(price):
    """Return % distance from nearest Gann level"""
    nearest = gann_nearest_level(price)
    return np.abs(price - nearest) / np.maximum(price, 0.01) * 100

def gann_all_levels(anchor_price, n=16):
    """Return n Gann levels above and below anchor"""
    sq = np.sqrt(anchor_price)
    return [(sq + i * GANN_UNIT)**2 for i in range(-n, n+1)]

# Thresholds
EMA21_GANN_TOL = 0.30   # EMA21 must be within 0.30% of a Gann level
CLOSE_GANN_TOL = 0.40   # close price within 0.40% (bonus confluence)

PREV={'trades':1299,'wr':30.79,'pf':1.14,'net_pct':109.68,'max_dd':22.75,'sharpe':0.80}
...
lunar_df['lunar_pct']=lunar_df['date'].apply(lambda d: get_lunar_illumination(d.strftime('%Y/%m/%d')))
...
# ── Load & Indicators ─────────────────────────────────────────────────────────
print("Loading data & computing indicators + Gann levels...")
...
# ✅ GANN SQUARE OF 9: compute distance of EMA21 and Close from nearest Gann level
df['gann_ema21_dist'] = df['ema21'].apply(gann_distance_pct)
df['gann_close_dist'] = df['close'].apply(gann_distance_pct)
df['gann_ema21_near'] = df['gann_ema21_dist'] < EMA21_GANN_TOL    # EMA21 at Gann level
df['gann_close_near'] = df['gann_close_dist'] < CLOSE_GANN_TOL    # Close at Gann level
df['gann_confluence'] = df['gann_ema21_near'] | df['gann_close_near']  # either or both

n_ema_gann  = df['gann_ema21_near'].sum()
n_close_gann= df['gann_close_near'].sum()
n_both      = df['gann_confluence'].sum()
print(f"  EMA21 near Gann level (<{EMA21_GANN_TOL}%): {n_ema_gann:,} bars ({n_ema_gann/len(df)*100:.1f}%)")
print(f"  Close near Gann level (<{CLOSE_GANN_TOL}%): {n_close_gann:,} bars ({n_close_gann/len(df)*100:.1f}%)")
print(f"  Either confluence:                  {n_both:,} bars ({n_both/len(df)*100:.1f}%)")

# Show sample Gann levels around mid-range gold price
sample_price=2000.0
sample_levels=[round(l,2) for l in gann_all_levels(sample_price, n=5)]
print(f"  Sample Gann levels around $2000: {sample_levels}")

# Merge lunar
df=df.merge(lunar_df[['full_moon_avoid']],left_on='date',right_index=True,how='left')
...
df['lunar_ok']=~df['full_moon_avoid']

# ✅ Filter #9: Gann confluence required
df['entry_long'] =(df['in_long_tap']  &(df['score_long']>=5)  &df['valid_session']&
                   df['macro_long']   &df['lunar_ok'] &df['gann_confluence'])
df['entry_short']=(df['in_short_tap'] &(df['score_short']>=5) &df['valid_session']&
                   df['macro_short']  &df['lunar_ok'] &df['gann_confluence'])

# ── Simulation ────────────────────────────────────────────────────────────────
print("Running backtest F#1+F#3+F#8+F#9...")
...
tdf.to_csv('xauusd_filter9_trades.csv',index=False)
...
print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #9: GANN SQUARE OF 9 CONFLUENCE
  EMA21 within {EMA21_GANN_TOL}% OR Close within {CLOSE_GANN_TOL}% of Gann level
╠══════════════════════╦══════════════╦═══════════════════════════╣
  Metric              │ F#1+F#3+F#8  │ +F#9 (Gann) Result
╠══════════════════════╬══════════════╬═══════════════════════════╣
  Total Trades        │  1,299       │  {len(tdf):,}  {chk(len(tdf),1299,False)} ({1299-len(tdf)} blocked)
...
print(f"""╠══════════════════════════════════════════════════════════════════╣
  Gann hits: EMA21={n_ema_gann:,} bars · Close={n_close_gann:,} bars · Either={n_both:,} bars
...
fig.suptitle(
    f"Filter #9: Gann Square of 9 Confluence  [F#1+F#3+F#8+F#9]\n"
    f"Net: {net/INITIAL*100:+.2f}%  ·  MaxDD: {max_dd:.1f}%  ·  Sharpe: {sharpe:.2f}  ·  "
    f"WR: {wr:.1f}%  ·  PF: {pf:.2f}×  ·  Trades: {len(tdf):,}  ({1299-len(tdf)} blocked by Gann filter)\n"
...
ax0.set_title(f'Equity — F#1+F#3+F#8+F#9  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
...
out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter9_gann.png'

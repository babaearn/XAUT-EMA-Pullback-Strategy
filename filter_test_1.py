"""
ITERATIVE FILTER TESTING FRAMEWORK
===================================
Baseline (verified, lookahead-fixed):
  Trades=2887 · WR=29.41% · PF=1.04x · Net=+86.75% · MaxDD=43.49% · Sharpe=0.40

Filter being tested: #1 — DAILY EMA200 MACRO FILTER
Rule:
  - Long entries only allowed when close > Daily EMA200 (macro uptrend)
  - Short entries only allowed when close < Daily EMA200 (macro downtrend)
  - Daily EMA200 is shifted by 1 bar BEFORE ffill (no lookahead)

Hypothesis: This should eliminate 2021's -10.45% losing year where gold
was in a macro correction below its 200-day EMA.
"""
...
BASELINE = {
    ...
    'trades':  2887, 'wr': 29.41, 'pf': 1.04,
    'net_pct': 86.75, 'max_dd': 43.49, 'sharpe': 0.40,
}
...
# ─── 1H Bias (with shift-1 lookahead fix — same as verified baseline) ────────
print("Computing 1H bias indicators (shift-1 fix)...")
...
# ─── ✅ FILTER #1: DAILY EMA200 — with shift-1 lookahead fix ─────────────────
print("Computing Daily EMA200 (with shift-1 fix)...")
d1 = df[['open','high','low','close','volume']].resample('1D').agg(
    {'open':'first','high':'max','low':'min','close':'last','volume':'sum'}).dropna()
d1['ema200'] = ema_c(d1['close'], 200)
d1_s = d1[['ema200']].shift(1)   # ✅ No lookahead
df['d1_ema200'] = d1_s['ema200'].reindex(df.index, method='ffill')

df.reset_index(inplace=True)

# ─── Scoring (same as baseline) ───────────────────────────────────────────────
d = df
...
# ✅ NEW: Daily EMA200 macro direction gate
df['macro_long']   = df['close'] > df['d1_ema200']   # only longs in uptrend
df['macro_short']  = df['close'] < df['d1_ema200']   # only shorts in downtrend
...
# ─── Simulation ───────────────────────────────────────────────────────────────
print("Running backtest with Filter #1: Daily EMA200...")
...
for i in range(300, len(df)):
    ...
    risk_amt = INITIAL * risk_pct   # ← always fixed on INITIAL ($10K)
    ...
    if pos is None:
        if eL[i]:
            sc = int(sL_a[i]); risk_pct = RISK_MAP.get(sc, 0.008)
            pos="long"; entry_score=sc; entry_t=ts_a[i]
            sl=c-ATR_SL*at; tp=c+(c-sl)*RR; score_log.append(sc)
        elif eS[i]:
            sc = int(sS_a[i]); risk_pct = RISK_MAP.get(sc, 0.008)
            pos="short"; entry_score=sc; entry_t=ts_a[i]
...
tdf.to_csv('xauusd_filter1_trades.csv', index=False)
...
def delta(new, old, higher_better=True):
    d = new - old
    icon = ('✅' if d > 0 else '❌') if higher_better else ('✅' if d < 0 else '❌')
    return f"{icon} {d:+.2f}"

print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #1 TEST: DAILY EMA200 MACRO DIRECTION
  Rule: Longs only when price > Daily EMA200 | Shorts when below
╠════════════════════════════╦══════════════╦════════════════════╣
  Metric                    │  Baseline    │  Filter #1 Result
╠════════════════════════════╬══════════════╬════════════════════╣
  Total Trades              │  2,887       │  {len(tdf):,}  {delta(len(tdf), 2887, False)}
  Win Rate                  │  29.41%      │  {wr:.2f}%  {delta(wr, 29.41)}
  Profit Factor             │  1.04x       │  {pf:.2f}x  {delta(pf, 1.04)}
  Net PnL                   │  +86.75%     │  {net/INITIAL*100:+.2f}%  {delta(net/INITIAL*100, 86.75)}
  Final Equity              │  $18,675     │  ${final_eq:,.0f}
  Max Drawdown              │  43.49%      │  {max_dd:.2f}%  {delta(max_dd, 43.49, False)}
  Sharpe Ratio              │  0.40        │  {sharpe:.2f}  {delta(sharpe, 0.40)}
  Avg Trades/Week           │  13.2        │  {wk_counts.mean():.1f}  
╠════════════════════════════╩══════════════╩════════════════════╣
  Yearly Breakdown:""")
for yr,v in yearly_pct.items():
    base_yr = {'2021':-10.45,'2022':6.40,'2023':8.45,'2024':57.20,'2025':19.90,'2026':5.25}
    bv = base_yr.get(str(yr), 0)
    diff_icon = '✅' if v > bv else '❌'
    print(f"    {yr}: {v:+.2f}%  (was {bv:+.2f}%)  {diff_icon}")
...
ax0.plot(eq_times, eq_s.values, color='#00e676', lw=1.2, label='Filter #1 Equity')
...
ax0.set_title(f'Equity Curve — Filter #1: Daily EMA200  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
...
styled_bar(ax1,sd['s'],sd['pct'].tolist(),'Net Profit (%) by Session',fs=9)

# Day
ax2=fig.add_subplot(gs[2,0])
day_order=['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']
dd2=tdf.groupby('dow').apply(calc_pct).reindex(day_order).fillna(0).reset_index(); dd2.columns=['d','pct']
styled_bar(ax2,dd2['d'],dd2['pct'].tolist(),'Net Profit (%) by Day',rot=30)
...
styled_bar(ax4,yd['y'].astype(str),yd['pct'].tolist(),'Yearly Net % — Filter #1 vs Baseline')
...
ax5.set_title(f'Weekly PnL Heatmap  (Avg {wk_counts.mean():.1f} trades/wk)',
              fontsize=9,fontweight='bold',color='white',pad=6)
ax5.set_facecolor('#0d0d1f'); ax5.tick_params(colors='#bbb',labelsize=7)

out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter1_daily_ema200.png'

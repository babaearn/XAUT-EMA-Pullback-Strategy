"""
FILTER TEST #7 — CANDLESTICK REVERSAL AT EMA TAP ZONE
=======================================================
...
Filter #7 Rule — require ONE of these bullish patterns at long entry:
  a) HAMMER / PIN BAR:
     Lower wick >= 60% of total candle range
     Body <= 35% of total candle range (small body = indecision → reversal)
     
  b) BULLISH ENGULFING:
     Current candle is bullish (close > open)
     Current body completely engulfs previous candle's body

  Mirror logic for shorts (shooting star / bearish engulfing)

Why this works:
  We already KNOW price is in the 0.2% EMA21 tap zone.
  A pin bar AT the EMA shows price TESTED the level and was REJECTED.
  That rejection candle is market structure proof of the EMA holding.
  Without a reversal candle, it's just price drifting through the EMA.
"""
...
PIN_WICK_MIN  = 0.60   # lower wick must be >= 60% of total range (hammer)
PIN_BODY_MAX  = 0.35   # body must be <= 35% of range (small body)
...
# Safe range (avoid div/0 on doji)
safe_range = df['candle_range'].replace(0, np.nan)

# ✅ Bullish Pin Bar / Hammer
df['bull_pin'] = ((df['lower_wick'] / safe_range >= PIN_WICK_MIN) &
                  (df['body'] / safe_range <= PIN_BODY_MAX) &
                  (df['candle_range'] > 0))

# ✅ Bullish Engulfing (current candle engulfs previous candle's body)
df['bull_engulf'] = (df['is_bullish'] &
                     (df['close'] > df['open'].shift(1)) &
                     (df['open'] < df['close'].shift(1)) &
                     (df['close'].shift(1) < df['open'].shift(1)))  # prev was bearish

# ✅ Bearish Pin Bar / Shooting Star
df['bear_pin'] = ((df['upper_wick'] / safe_range >= PIN_WICK_MIN) &
                  (df['body'] / safe_range <= PIN_BODY_MAX) &
                  (df['candle_range'] > 0))

# ✅ Bearish Engulfing
df['bear_engulf'] = (df['is_bearish'] &
                     (df['close'] < df['open'].shift(1)) &
                     (df['open'] > df['close'].shift(1)) &
                     (df['close'].shift(1) > df['open'].shift(1)))  # prev was bullish

df['bullish_reversal'] = df['bull_pin'] | df['bull_engulf']
df['bearish_reversal'] = df['bear_pin'] | df['bear_engulf']

# Log how many patterns detected
n_bull = df['bullish_reversal'].sum()
n_bear = df['bearish_reversal'].sum()
print(f"  Bullish reversals detected: {n_bull:,} ({n_bull/len(df)*100:.1f}% of bars)")
print(f"  Bearish reversals detected: {n_bear:,} ({n_bear/len(df)*100:.1f}% of bars)")
...
# ✅ Filter #7: Require bullish/bearish reversal candle at tap zone
df['entry_long'] =(df['in_long_tap']  &(df['score_long']>=5)  &df['valid_session']&
                   df['macro_long']   &df['bullish_reversal'])
df['entry_short']=(df['in_short_tap'] &(df['score_short']>=5) &df['valid_session']&
                   df['macro_short']  &df['bearish_reversal'])

# ── Simulation ────────────────────────────────────────────────────────────────
print("Running backtest F#1 + F#3 + F#7...")
...
tdf.to_csv('xauusd_filter7_trades.csv',index=False)
...
# Pattern breakdown
n_long=len(tdf[tdf['type']=='long']); n_short=len(tdf[tdf['type']=='short'])
...
print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #7: CANDLESTICK REVERSAL AT EMA TAP ZONE
  Pin bar (wick≥60% range, body≤35%) OR Engulfing candle
╠══════════════════════╦══════════════╦═══════════════════════════╣
  Metric              │ F#1+F#3 Base │ F#1+F#3+F#7 Result
╠══════════════════════╬══════════════╬═══════════════════════════╣
  Total Trades        │  1,742       │  {len(tdf):,}  {chk(len(tdf),1742,False)} ({1742-len(tdf)} blocked)
  Long / Short        │  —           │  {n_long} long / {n_short} short
...
╠══════════════════════╩══════════════╩═══════════════════════════╣
  Pattern Detection Rates:
    Bull Pin Bars:   {df['bull_pin'].sum():,}  |  Bull Engulfing: {df['bull_engulf'].sum():,}
    Bear Pin Bars:   {df['bear_pin'].sum():,}  |  Bear Engulfing: {df['bear_engulf'].sum():,}
  Yearly Breakdown:""")
...
fig.suptitle(
    f"Filter #7: Candlestick Reversal (Pin Bar / Engulfing at EMA21 Tap)  [F#1 + F#3 + F#7]\n"
...
ax0.set_title(f'Equity Curve — F#1+F#3+F#7  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
...
out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter7_candlestick.png'

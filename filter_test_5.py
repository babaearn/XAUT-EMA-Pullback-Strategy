"""
FILTER TEST #5 — RSI MOMENTUM (RISING OVER 2 BARS)
====================================================
Builds on: Filter #1 (Daily EMA200) + Filter #3 (Inv Vol Sizing) — both ACCEPTED
Active baseline: Trades=1,742 · WR=30.08% · PF=1.08x · Net=+86.81% · MaxDD=28.41% · Sharpe=0.56

Filter #5 Rule:
  Long:  rsi[i] > rsi[i-2]  (RSI turned up from pullback in last 10 min)
  Short: rsi[i] < rsi[i-2]  (RSI turning down from bounce in last 10 min)

Why 2 bars (not 1 or 3):
  1-bar: Too noisy on 5m (single-bar RSI bounce = whipsaw)
  2-bar: 10-min trend = meaningful confirmation of pullback ending
  3-bar: Tested via ADX rising → proved too restrictive

Logic: EMA pullback setups work best when the PULLBACK IS ENDING.
  RSI dipped during pullback → now turning back up = momentum resuming
  This is exactly the sweet spot for entry.

Also testing BONUS variant: RSI increasing AND RSI > 50 (for longs)
  This is already in the score system but now REQUIRED, not optional.
"""
...
PREV={'label':'F#1+F#3','trades':1742,'wr':30.08,'pf':1.08,
      'net_pct':86.81,'max_dd':28.41,'sharpe':0.56}
INITIAL=10_000.0; RR=2.5; ATR_SL=1.5
BASE_RISK={5:0.008,6:0.012,7:0.015}
VOL_FLOOR=0.4; VOL_CAP=2.0
RSI_LOOKBACK=2   # bars back for RSI momentum check
...
# ✅ Filter #5: RSI momentum (rising over N bars)
df['rsi_rising']  = df['rsi'] > df['rsi'].shift(RSI_LOOKBACK)   # for longs
df['rsi_falling'] = df['rsi'] < df['rsi'].shift(RSI_LOOKBACK)   # for shorts
...
s3L=d['macd_hist']>0; s4L=d['h1_ema21']>d['h1_ema200']
...
s3S=d['macd_hist']<0; s4S=d['h1_ema21']<d['h1_ema200']
...
# ✅ Apply Filter #5: RSI must be rising for longs / falling for shorts
df['entry_long']=(df['in_long_tap']  &(df['score_long']>=5)  &df['valid_session']&
                  df['macro_long']   &df['rsi_rising'])
df['entry_short']=(df['in_short_tap'] &(df['score_short']>=5) &df['valid_session']&
                   df['macro_short']  &df['rsi_falling'])

# ── Simulation ────────────────────────────────────────────────────────────────
print(f"Running backtest F#1 + F#3 + F#5 (RSI lookback={RSI_LOOKBACK} bars)...")
...
tdf.to_csv('xauusd_filter5_trades.csv',index=False)
...
blocked=1742-len(tdf)
print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #5: RSI MOMENTUM (RSI now > RSI {RSI_LOOKBACK} bars ago)
  Long: RSI rising | Short: RSI falling   (5m timeframe)
╠══════════════════════╦══════════════╦═══════════════════════════╣
  Metric              │ F#1+F#3 Base │ F#1+F#3+F#5 Result
╠══════════════════════╬══════════════╬═══════════════════════════╣
  Total Trades        │  1,742       │  {len(tdf):,}  {chk(len(tdf),1742,False)} ({blocked} blocked)
...
print(f"""╠══════════════════════════════════════════════════════════════════╣
  ...
  Avg risk: {np.mean(risk_log)*100:.3f}%  |  RSI lookback: {RSI_LOOKBACK} bars
╚══════════════════════════════════════════════════════════════════╝""")
...
fig.suptitle(
    f"Filter #5: RSI Momentum  (RSI now > RSI {RSI_LOOKBACK} bars ago)  [F#1 + F#3 + F#5]\n"
    f"Net: {net/INITIAL*100:+.2f}%  ·  MaxDD: {max_dd:.1f}%  ·  Sharpe: {sharpe:.2f}  ·  "
    f"WR: {wr:.1f}%  ·  PF: {pf:.2f}×  ·  Trades: {len(tdf):,}  ({blocked} blocked by F#5)\n"
...
ax0.set_title(f'Equity Curve — F#1+F#3+F#5  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
...
out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter5_rsi_momentum.png'

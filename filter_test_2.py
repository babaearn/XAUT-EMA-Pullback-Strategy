"""
FILTER TEST #2 — ATR HEALTHY VOLATILITY ZONE
=============================================
Builds on: Filter #1 (Daily EMA200) — ACCEPTED
New baseline: Trades=1,742 · WR=30.08% · PF=1.07x · Net=+88.60% · MaxDD=33.95% · Sharpe=0.51

Filter #2 Rule:
  ATR_avg × 1.0  <  current_ATR  <  ATR_avg × 2.5
  where ATR_avg = 50-bar rolling mean of ATR

Meaning:
  ✅ ATR above its average  → Market has real movement (not dead)
  ✅ ATR below 2.5× average → Not a news spike/flash crash (bad fill risk)
  
Hypothesis: Eliminating dead-market and spike entries reduces losses from
flat chop periods and high-slippage events.
"""
...
PREV = {'label':'Filter #1 (Daily EMA200)',
        'trades':1742,'wr':30.08,'pf':1.07,
...
# ✅ FILTER #2: ATR 50-bar rolling average for healthy zone
...
# ✅ Filter #2: ATR healthy zone (not too cold, not too spiked)
ATR_LOW  = 1.0   # must be above avg ATR (market moving)
ATR_HIGH = 2.5   # must be below 2.5× avg ATR (not a spike event)
df['atr_healthy'] = ((df['atr'] > df['atr_avg50'] * ATR_LOW) &
                     (df['atr'] < df['atr_avg50'] * ATR_HIGH) &
                     (df['atr_avg50'] > 0))

df['entry_long']  = (df['in_long_tap']  & (df['score_long']>=5)  &
                     df['valid_session'] & df['macro_long']  & df['atr_healthy'])
df['entry_short'] = (df['in_short_tap'] & (df['score_short']>=5) &
                     df['valid_session'] & df['macro_short'] & df['atr_healthy'])

# ── Simulation ────────────────────────────────────────────────────────────────
print("Running backtest with Filter #1 + #2...")
INITIAL=10_000.0; RR=2.5
RISK_MAP={5:0.008,6:0.012,7:0.015}; ATR_SL=1.5
...
tdf.to_csv('xauusd_filter2_trades.csv',index=False)
...
# ── Comparison ────────────────────────────────────────────────────────────────
def chk(new,old,higher_better=True):
    d=new-old
...
print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #2 TEST: ATR HEALTHY ZONE ({ATR_LOW}×avg < ATR < {ATR_HIGH}×avg)
  Added on top of Filter #1 (Daily EMA200)
╠══════════════════════╦═════════════╦════════════════════════════╣
  Metric              │ F#1 Baseline│ F#1 + F#2 Result
╠══════════════════════╬═════════════╬════════════════════════════╣
  Total Trades        │  1,742      │  {len(tdf):,}  {chk(len(tdf),1742,False)}
...
print(f"""╠══════════════════════════════════════════════════════════════════╣
  ATR stats: healthy bars / total bars checked
...
fig.suptitle(
    f"Filter #2 Test: ATR Healthy Zone ({ATR_LOW}×avg < ATR < {ATR_HIGH}×avg)  [Built on Filter #1]\n"
...
ax0.set_title(f'Equity Curve — F#1+F#2  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
...
out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter2_atr_zone.png'

"""
FILTER TEST #4 — ADX RISING FOR 3 CONSECUTIVE 1H BARS
=======================================================
...
Filter #4 Rule:
  h1_adx[t] > h1_adx[t-1] > h1_adx[t-2]   (3-bar rising ADX on 1H)
  → Still requires adx > 20 AND direction (PDI/NDI) — rising ACCELERATION, not just level

Why on 1H not 5m:
  5m ADX oscillates too rapidly (15-min rising streak is noise)
  1H rising 3 bars = 3 hours of trend strengthening = real momentum build

Hypothesis:
  Filters setups where trend is fading/topping. Best EMA pullback setups
  occur when the trend is actively strengthening, not decelerating.
"""
...
# ── 1H bias with shift-1 AND ADX rising check ────────────────────────────────
print("Computing 1H indicators + ADX rising filter...")
...
# ✅ Filter #4: ADX rising for 3 consecutive bars
h1['adx_rising'] = ((h1['adx'] > h1['adx'].shift(1)) &
                    (h1['adx'].shift(1) > h1['adx'].shift(2)))

# Shift ALL of this by 1 bar (no lookahead) before ffilling
h1_s=h1[['ema21','ema200','rsi','adx','pdi','ndi','adx_rising']].shift(1)
...
df['h1_adx_rising']= h1_s['adx_rising'].reindex(df.index,method='ffill').fillna(False)
...
# ✅ Apply Filter #4 to entry conditions
df['entry_long']=(df['in_long_tap']  & (df['score_long']>=5)  & df['valid_session'] &
                  df['macro_long']   & df['h1_adx_rising'])
df['entry_short']=(df['in_short_tap'] & (df['score_short']>=5) & df['valid_session'] &
                   df['macro_short']  & df['h1_adx_rising'])

# ── Simulation (F#1 + F#3 + F#4) ────────────────────────────────────────────
print("Running backtest F#1 + F#3 + F#4...")
...
tdf.to_csv('xauusd_filter4_trades.csv',index=False)
...
print(f"""
╔══════════════════════════════════════════════════════════════════╗
  FILTER #4: ADX RISING 3 CONSECUTIVE 1H BARS
  (Built on F#1 + F#3 stack)
╠══════════════════════╦══════════════╦═══════════════════════════╣
  Metric              │ F#1+F#3 Base │ F#1+F#3+F#4 Result
╠══════════════════════╬══════════════╬═══════════════════════════╣
...
print(f"""╠══════════════════════════════════════════════════════════════════╣
  ...
  ADX-rising filtered out: {1742-len(tdf)} trades
╚══════════════════════════════════════════════════════════════════╝
""")
...
fig.suptitle(
    f"Filter #4: ADX Rising 3 Consecutive 1H Bars  [F#1 + F#3 + F#4]\n"
...
ax0.set_title(f'Equity Curve — F#1+F#3+F#4  |  Final ${final_eq:,.0f}  |  Max DD {max_dd:.1f}%',
              fontsize=10,fontweight='bold',color='white',pad=7)
ax0.set_facecolor('#0d0d1f'); ax0.spines[:].set_color('#333')
ax0.tick_params(colors='#bbb',labelsize=8); ax0.set_ylabel('Equity ($)',color='#aaa'); ax0.grid(alpha=0.12)
...
out='/Users/mudrex/.gemini/antigravity/brain/52dfe01a-90ef-4075-85d5-508cdf7ccaa5/filter4_adx_rising.png'
